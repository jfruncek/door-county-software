...
{	
	...
	private boolean isBuildForInitialDisplay() {
	    return !submitOk && !submitValidate && !submitDuplicate;
	}
	...
	public String execute()
	{
		try
		{
			...
			else // add is assumed to be the only other option
			{
				reportParameter = new ReportParameter();
				if (isBuildForInitialDisplay()) {
				    type = ReportParameter.TYPE_TEXT;
				    className = ReportParameter.STRING;
				}
			}
			...
			if (command.equals("edit"))
			{
				parameterProvider.updateReportParameter(reportParameter, reportUser.getName());
			}

			if (command.equals("add"))
			{
				reportParameter = parameterProvider.insertReportParameter(reportParameter, reportUser.getName());
                id = reportParameter.getId().intValue();
			}
