...
public class ReportUser implements CtrlDataObject, Serializable
{	
	...
	public Set<Report> getReports()
	{
		...
		while(iterator.hasNext())
		{
			...
			addReports(group.getReports(), set);
		}
	...
	/**
	 * Adds reports to the input Report list.
	 * 
	 * @param groupReports
	 */
	private void addReports(List<Report> groupReports, 	TreeSet<Report> reportList){
		for(Report groupReport : groupReports){
			if(groupReport != null){
				reportList.add(groupReport);
			}
		}
	}
	...
	{
		Iterator<ReportUserAlert> iterator = alerts.iterator();
		while (iterator.hasNext())
		{
			ReportUserAlert map = iterator.next();
	...
	public boolean isBookAdmin() {
		return roles.contains(ORStatics.BOOK_ADMIN_ROLE) || isRootAdmin();
	}

	public void setBookAdmin(boolean hasRole) {
		roles.remove(ORStatics.BOOK_ADMIN_ROLE);
		if (hasRole) roles.add(ORStatics.BOOK_ADMIN_ROLE);
	}    
}